classdef UniformActionNoise < matlab.mixin.Copyable
    ...
    methods        
        function this = UniformActionNoise(ActionSize, NoiseOpts)
            % build the noise model with the size of the action space, the
            % agent sample time, and the agent noise options
            
            validateattributes(NoiseOpts, {'rl.option.UniformActionNoise'}, {'scalar', 'nonempty'}, mfilename, 'NoiseOpts', 2);
